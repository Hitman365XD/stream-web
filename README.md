...
Aplicación web alterna para transmisiones en vivo ligadas a Twitch, con información del canal emisor, chat incorporado e interfaz similar (vídeo fuente desde Okru / VK vídeo).
...
* **Lectura de API twitch para credenciales**
    ...
    Aquí es ejecutar un [servicio de vídeo](https://github.com/Hitman365XD/mini-stream-server), que envía la información del canal en Twitch. Deben ser colocados en un .env, que tendrá la misma estructura de `.env.example` (VITE_DATA_URL el `/stream-data`).
...
**<u>Nota</u>:** Para acceder al chat de Twitch, se debe permitir las cookies de terceros en su navegador correspondiente (PC y móvil), caso contrario no permitirá el log in.
